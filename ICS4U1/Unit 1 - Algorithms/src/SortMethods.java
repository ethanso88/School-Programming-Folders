...
//            data[smallest] = temp;
        ...
        for (int x = 0; x < msd; x++) {
            ...
            for (int y = 0; y < 10; y++)
                ...
                while (!digit[y].isEmpty()) {
                    //add it to the original array and then wipe the memory of the number added to move to the next number in the arraylist or that place value
    ...
    public static void shellSort(int[] data) {
        ...
        //create flag to check for swaps
        boolean swapped;
        //starting gap length between compared numebrs
        int gap = data.length / 2;

        //while a swap has occurred and gap length is greater than 0
        do {
            do {
                //set swapped to false before each pass
                swapped = false;

                //iterate through array comparing value with the element gap length away from it
    ...
    public static void mergeSort(int[] data, int start, int end) {
        // Base case: If there is only one or zero elements, the array is already
        // sorted.
        if ((end - start) >= 1) {
    ...
    private static void merge(int[] data, int start, int end) {
        ...
        // Index for inserting sorted values
        int mergeIndex = 0;

        // Find the middle index to separate left and right subarrays
        int middle = (start + end) / 2;
        int leftIndex = start; // Starting index of left subarray
        ...
        while (leftIndex <= middle && rightIndex <= end) {
            if (data[leftIndex] < data[rightIndex]) {
                mergedSubArray[mergeIndex] = data[leftIndex]; // Take element from left subarray
                leftIndex++; // Move left index forward
            } else {
                mergedSubArray[mergeIndex] = data[rightIndex]; // Take element from right subarray
                rightIndex++; // Move right index forward
            }
            mergeIndex++; // Move to next position in merged array
        }

        // If left subarray has remaining elements, add them to merged array
        if (leftIndex > middle) {
        ...
        } else { // If right subarray has remaining elements, add them to merged array
